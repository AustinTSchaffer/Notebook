...
We know that on the simplest of examples, Dijkstra simply can't handle edges with negatives, and we don't want to modify absolute path lengths. Can we inject new vertices/edges into G which bypass the negative edge(s)?

Let's consider a more complicated graph. The answer is still pretty much the same in this graph, just with extra noise to consider.
