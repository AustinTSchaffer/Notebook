---
tags:
  - OMSCS
  - Algorithms
  - Practice
---
# Suggested Graph Problems

## Chapter 3
- **3.1** – Perform a depth-first search on the following graph...
- **3.2** – Perform depth-first search on each of the following graphs...
- **3.7(a-c)** – A _bipartite graph_ is a graph...
- **3.8** – _Pouring Water_
- **3.11** – Design a linear-time algorithm...
- **3.16** – Suppose a CS curriculum...
	1. Modified BFS (outside the scope of the course)  
	2. ~~Solution with a propagation step~~ [[3.16 - CS Curriculum]]
- **3.22** – Give an efficient algorithm which takes as input a directed graph $G=(V,E)$...
- **3.24** – Give a linear-time algorithm for the following task.

## Chapter 4 (BFS, Dijkstra's, shortest paths)
- **4.1** – Suppose Dijkstra's algorithm is run...
- **4.2** – Just like the previous problem, but this time with the Bellman-Ford algorithm.
- **4.3** – _Squares._
- **4.8** – Professor F. Lake suggests...
- **4.9** –  Consider a directed graph in which the only negative edges... [[4.9 - Dijkstra with Some Negative Edges (TODO)]]
- **4.11** – Give an algorithm that takes as input a directed graph with positive edge lengths...
- **4.12** – Give an $O(|V|^2)$ algorithm...
	- It is recommended to ignore the target runtime which is based on one of the algorithm variations outside the scope of the course.
- **4.13** – You are given a set of cities...
- **4.20** – There is a network of roads $G=(V,E)$...
- ~~**4.21** – Shortest path algorithms can be applied in currency trading.~~ [[4.21 - Currency Exchange]]

