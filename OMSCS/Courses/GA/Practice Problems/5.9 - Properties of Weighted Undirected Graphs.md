...
- **Useful Information**
	...
	- Prim’s algorithm and Kruskal's algorithm both work correctly when there are negative edges.
...
True. This one requires some work. Thankfully it was pivotal for a recent assignment, and the result is 
...
Running Kruskal's on the graph produces this MST. Total edge weight is 6.
...
This is not true. Review the same graphs from problem G. The shortest path from A to C in $G$ has cost 5. In the MST, the path from A to C has cost 6.

## Problem I
> Prim’s algorithm works correctly when there are negative edges.

Yes. The only requirement is minimizing the sum of edge weights.
